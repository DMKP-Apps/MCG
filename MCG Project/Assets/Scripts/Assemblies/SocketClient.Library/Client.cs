...
namespace SocketClient.Library
{
    public class Client
    {
        ...
        private SocketResponse GetResponse(string data)
        {
            if (data == null || data.IndexOf("<SDATA>") != 0 || data.IndexOf("</SDATA>") != data.Length - "</SDATA>".Length)
                return new SocketResponse() { Data = null };
            ...
            if (string.IsNullOrEmpty(data))
                return new SocketResponse() { Data = null };
            var result = data.FromJsonString<SocketResponse>();
            return result;// ConvertResponse<T>(result);
        ...
        public void SendRequest<TRequest>(SocketRequest<TRequest> request)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        _onRequestComplete(GetResponse(data));
                    };
                    Action<Exception> onError = (error) =>
                    {
                        _onRequestComplete(new SocketResponse()
                        {
                            Status = false,
        ...
        Action<SocketResponse> _onRequestComplete = (data) => { };
        private string _host = string.Empty;
        private int _port = 11000;
        public bool StartClient(string host, int port, Action<SocketResponse> onRequestComplete)
        {
